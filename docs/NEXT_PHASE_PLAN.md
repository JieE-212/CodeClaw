...
CodeClaw 已完成第一轮真人测试、第一批问题修复以及 `Stage 3.0.10` 至 `4B` 的授权机器工程循环。真人复测继续暂停；自动循环在 4B machine candidate 停止，`4C` 延后且未开始。
...
| 4B | 降低本地启动和诊断摩擦 | 当前仅机器验证 | machine candidate 已完成；真实 Windows 人工验收仍待完成 |
...
原计划把 3–5 次真人试用作为投入门槛；2026-07-12 用户授权在暂停真人测试期间提前完成 4B 工程候选。该授权任务现已完成机器实现和提交，自动循环停在这里：

- `stage4b:machine` 只接受 clean commit，并只从该提交的 Git blob 构建候选；ignored/untracked 文件不会进入包。
- Authority JSON、SHA-256 sidecar、候选 ID、完整文件/空目录清单和 Windows 便携路径门禁会阻断缺失、额外、变化、链接、硬链接、大小写/NFC 冲突和预算超限。
- 启动器验证 Node 20+ 与完整候选后才写外部 runtime、spawn 或开浏览器；默认预扫 4173–4199，显式端口占用则 fail closed。
- 可写 Demo 和所有状态都在 LocalAppData 候选级目录，不修改被 Authority 覆盖的 package；候选内绕过 launcher 的直接 server 启动被拒绝。
- candidate/instance/server PID/port 使用 challenge HMAC 绑定；旧标签页的每个 API 请求都由服务端复验。独立 status/stop、重启、失败清理和无秘密诊断都有机器回归。
- legacy local-trial package 已降级为不可启动、不可分享的历史流程回归包。
...
| 4B Windows 启动器与候选包 | machine verified；committed；manual Windows acceptance pending | tracked-commit package、Authority/sidecar、外部 runtime Demo、回环监听、端口预扫/分流、逐请求实例绑定、HMAC stop、启动/停止/重启与清理门禁 | 签名安装器或最终 Windows 发布；干净 Windows 10/11、Defender/SmartScreen、默认浏览器、真实双击、NVDA/高对比度等人工验收 |
...
当前正式状态：`Stage 4B machine verified and committed; manual Windows acceptance pending; remediation REMEDIATION_HOLD`。host-1 七项人工验收仍待完成，真人 tester-3 保持 `not scheduled`，真实原项目写入没有因此开放。
...
Stage 3.0.14 的最终 diff/Git/禁入项/临时产物审计与独立提交已完成；`dist/`、`.codeclaw/`、日志、截图、真人记录或证据包均未提交，也未保留临时测试代码或墓碑分支。随后 Stage 4B 也已完成，不直接 push。

### 15.7 Stage 4B 已完成的机器证据

当前状态是 `machine verified; committed; manual Windows acceptance pending`。唯一完整机器门禁是：

```powershell
npm.cmd run stage4b:machine
```

它要求 clean 40-hex commit，先运行 source check、launcher 聚焦测试和 source health；随后从 Git object database 的 tracked blob 创建 pending candidate，运行 candidate check，通过真实 candidate launcher 完成 start/status/stop/restart、外置 Demo 路径和旧标签身份验证，再复验 Authority 并原子发布最终目录。失败时不能留下看似 ready 的 final candidate，无法证明安全删除时会明确报 cleanup blocker。

机器实现还覆盖：Node 20+、零验证前 spawn/浏览器/runtime 写入、4173–4199 全范围同候选预扫、显式端口 fail closed、HMAC 绑定 candidate/instance/PID/port、服务 ready 后才打开浏览器、控制文件与 start/stop 互斥、reserved -> PID 持久化 -> stdin nonce gate 的崩溃安全启动顺序、dead/rebooted reservation 恢复、终止树未验证时保留控制证据、HTTP redirect/URL/type/size/deadline 拒绝、项目命令剥离 launcher bearer capability、可写 Demo 不改变 candidate、逐请求旧标签保护，以及 direct candidate server 绕行阻断。

最终机器证据：Stage 4B 聚焦门禁 84 total、81 pass、0 fail、3 个环境性 skip；默认并发与单并发全量均为 476 total、469 pass、0 fail、7 个环境性 skip；`check`/i18n（每种语言 724 keys、0 warning/failure）、health、smoke、四个 pilot、明确路径的真实示例仓库只读 preflight、wrapper 语法和清理审计通过。独立安全复审没有剩余 High/Medium。

Stage 4B 的提前完成没有跳过或授权 Stage 3.1、3.2、4A，也没有自动开始 4C。真人测试、host-1 主观验收、干净 Windows、非管理员、Defender/SmartScreen、默认浏览器、双击、真实 `taskkill /T`、像素、完整键盘、NVDA 和高对比度继续保留为人工项。tester-2 保持 `AFTER_LIVE_BLOCKED`，remediation 保持 `REMEDIATION_HOLD`，tester-3 保持 `not scheduled`，原项目继续只读。
