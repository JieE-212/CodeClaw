...
async function writeManifest(copied) {
  const manifest = [
    ...
    "For hosted first trials, keep `docs/TRIAL_HOST_BRIEF.md`, `docs/TRIAL_GO_NO_GO.md`, `docs/TRIAL_5_MIN_PRECHECK.md`, and a generated `trial:session-pack` folder open. Run `npm.cmd run trial:intake-review-dry-run` once before filling the first real tester roster. Run `npm.cmd run trial:intake` before generating a real tester session pack, then run `npm.cmd run trial:intake-session -- --force`. Run `npm.cmd run trial:status` whenever the next step is unclear. Run `npm.cmd run trial:host-ready` immediately before hosting, then run `npm.cmd run trial:host-run` and use the generated `HOST_RUNBOOK.md` live. Run `npm.cmd run trial:pre-live` before scheduling or starting the real tester session. Run `npm.cmd run trial:live-capture` before the call and use the generated capture files. Ask testers to fill the generated feedback files afterward, then run `npm.cmd run trial:after-live` to complete completion, privacy, post-session, review, archive, status, and local evidence packaging. After at least two completed tester folders exist, run `npm.cmd run trial:cohort-summary` before expanding to 3-5 testers. Keep `trial:archive-session` available for manual local-only archive repair after privacy passes.",
    "",
    "## Exclusions",
    "",
    "- `.git/`",
