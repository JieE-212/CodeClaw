...
async function writeManifest(copied) {
  const manifest = [
    ...
    "For hosted first trials, keep `docs/TRIAL_HOST_BRIEF.md`, `docs/TRIAL_GO_NO_GO.md`, `docs/TRIAL_5_MIN_PRECHECK.md`, and a generated `trial:session-pack` folder open. Run `npm.cmd run trial:host-ready` immediately before hosting. Ask testers to fill the generated feedback files afterward, then run `npm.cmd run trial:ingest-feedback` plus `npm.cmd run trial:fix-backlog` before inviting tester 2.",
    "",
    "## Exclusions",
    "",
    "- `.git/`",
